...
// ===== Multi-line / area chart with hover tooltip =====
function AreaChart({ series, labels, height = 240, currency = false, gridLines = 5, showAxis = true }) {
  ...
  function onMove(e) {
    ...
    const xVal = pct * W;
    const idx = Math.max(0, Math.min(labels.length - 1, Math.round((xVal - padding.l) / stepX)));
    setHover({ idx, px });
  }
  ...
  return (
    <div className="chart-wrap" ref={wrapRef} onMouseMove={onMove} onMouseLeave={onLeave}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} width="100%" height={height} preserveAspectRatio="none" style={{ display: 'block' }}>
        <defs>
          {series.map((s, i) => (
        ...
        {hover && series.map((s, i) => (
          <circle key={i} cx={x(hover.idx)} cy={y(s.values[hover.idx])} r="4" fill="#fff" stroke={s.color} strokeWidth="2"/>
        ))}
      </svg>
      ...
      {hover && (
        <>
          <div className="chart-cursor" style={{ left: `${(x(hover.idx) / W) * 100}%` }}/>
          <div className="chart-tooltip" style={{
            left: `${(x(hover.idx) / W) * 100}%`,
            top: 8,
          }}>
            <div style={{ fontWeight: 700, marginBottom: 4, opacity: .8 }}>{labels[hover.idx] || `Pt ${hover.idx + 1}`}</div>
            {series.map((s, i) => (
              <div className="row" key={i}>
                <span style={{ display: 'inline-block', width: 8, height: 8, background: s.color, borderRadius: 2 }}></span>
                <span style={{ opacity: .8 }}>{s.name}</span>
                <b>{currency ? '$' + fmt(s.values[hover.idx], { compact: true }) : fmt(s.values[hover.idx])}</b>
              </div>
            ))}
          </div>
        </>
